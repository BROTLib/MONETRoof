<#
.SYNOPSIS
  Deploys MonetRoofTests to a TwinCAT runtime, runs the TcUnit suites and reports the result.
...
.DESCRIPTION
  ...
  reports the counters. Build first (TcBuild build MonetRoofTests.sln) so the compiled project is current.
...
[CmdletBinding()]
param(
    [string]$Solution    = (Join-Path $PSScriptRoot '..\MonetRoofTests.sln'),
    [string]$TargetNetId = '192.168.4.1.1.1',
    [string]$PlcTreePath = 'TIPC^MonetRoofTests',
