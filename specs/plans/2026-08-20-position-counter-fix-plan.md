...
**Status: implemented and merged in the shared repo — the 2026-09-15 "contradicted" re-check was accurate for one stale local clone, not for `origin/main`. Corrected 2026-09-16.**

Re-verified 2026-09-16 on a freshly `git fetch --tags`'d clone:
- `git tag -l` shows `v1.1.0` and `v1.1.1` both exist on `origin`.
- `git merge-base --is-ancestor v1.1.1 origin/main` → **true** (exit 0) — the fix is a direct ancestor of the actual current `origin/main` tip.
- `origin/main`'s actual current HEAD (`82512e5`) is only 2 commits ahead of the `v1.1.1` merge (`fix/roofmotor-position-write`, PR #2) — both PR #1 (`fix/roof-counter-resync`) and PR #2 merges are plainly visible in `git log main --oneline`.
- Confirmed directly in the working files: `FB_RoofMotor.TcPOU`/`FB_Roof.TcPOU` at current `main` contain `SetPosition`, `tonDebounce`, `counter_debounce` — the fix's actual code, not the pre-fix unfiltered counter.

So the fix genuinely was committed, merged (PRs #1, #2, #3 all real and merged), and tagged — `origin/main`, the shared repo everyone should be pulling from, is not missing it.

**What the 2026-09-15 check most likely actually found**: not "the fix doesn't exist," but that *one specific local clone* — plausibly MONET/N's own `Becky` (a separate physical machine per the naming-convention note) — was stuck at `b395c89` (2026-01-07) because nobody had pulled on that machine since January. That's a real, still-open finding, just narrower than originally stated: **MONET/N's local MONETRoof checkout (and whatever's actually deployed to MONET/N's roof controller from it) may still be running the pre-fix code**, even though the shared repo has moved on. Worth checking directly on that machine — both its git state (`git fetch && git log --oneline main` to see how far behind it actually is) and, more importantly, what's actually deployed to the running controller (source-tree presence on a dev PC doesn't mean it's been built and downloaded to hardware — check live via ADS, matching the pattern in IAG's own private specs, `design/monets-live-ads-diagnostics.md`). MONET/S's own roof, and any machine that *did* pull recently, should already have the fix in source at least.

<details>
<summary>Superseded 2026-09-15 "contradicted" note (wrong, kept for history)</summary>

Status: CONTRADICTED BY REPO — re-opened 2026-09-15. This doc claims the
fix below (steps 3–4) was implemented on branches `fix/roof-counter-resync`
(2026-08-20) and `fix/roofmotor-position-write` (2026-08-21) and validated live
on 2026-08-21. Verified against `MONETRoof` on 2026-09-15: neither branch
exists in the repo (checked `git branch -a` and `git log --all --oneline` —
no match for `counter_debounce`, `tonDebounce`, `SetPosition`, or any of the
identifiers this doc describes, anywhere in history). The repo's last commit
(`b395c89`, 2026-01-07) predates this plan's claimed work by over seven
months, and `FB_RoofMotor.TcPOU`/`FB_Roof.TcPOU` at `main` HEAD still contain
exactly the unfiltered, ungated counter and the commented-out limit re-sync
described in §1/§2 below as the problem. The fix described here was never
committed — either the branches were made and lost (not pushed / local-only
and later wiped) or the validation happened against code that was never
merged back. Either way, the roof position-counter bug this plan set out to
fix is very likely still live in production. Needs re-investigation before
trusting the roof's automatic sync/limit behavior; do not treat the
"implemented"/"validated" claims in the rest of this document as current.

Leading hypothesis (unverified): this doc never states which telescope's
`Becky` the fix was built/validated on — just "Becky" — and per
the Becky naming-convention note, every telescope's dev PC shares that name.
The repo check above only covers MONETRoof's remote/shared git history; it
says nothing about local, unpushed state on whichever physical Becky did the
work. If that machine was MONET/N's, the fix may still exist there as
uncommitted/unpushed local branches (possibly even deployed to that
controller) and simply never got pushed to the shared repo — a "push it"
problem, not a "the fix never existed" problem. Needs checking directly on
MONET/N's `Becky` for unpushed branches or uncommitted changes before
assuming the fix must be rewritten from scratch.
...
<details>
<summary>Original status line (superseded, kept for history)</summary>

Status: implemented (steps 3–4) and validated on the live system (2026-08-21) — a full close and a
full open were completed on Becky; both limit snaps (`closed`/`opened`) engage without
`sync_error`/`limit_error`, including the first-ever recorded full open (opened-limit repeatability
closed, see §5). First live recording (`Roof.svdx`, Becky) decoded 2026-08-20 — measured values in
§3; decoding format documented in `BROTLib/specs/design/twincat-scopeview-svdx-format.md`.
